<?php
	...
	function gantt_alert_task_done($project_guid, $task_guid)
	{
		...
		if($project) {
			...
			$message = sprintf(elgg_echo('gantt:alert:task:done'),$task->title,$project->getURL(),$project->title);
	...
	function gantt_alert_task_blocked($project_guid, $task_guid, $reason)
	{
		...
		if($project) {
			...
			$message = sprintf(elgg_echo('gantt:alert:task:blocked'),$task->title,$project->getURL(), $project->title,$reason);
			notify_user($project->project_manager, $project->container_guid,$title,$message);
		}
		return false;
	}
